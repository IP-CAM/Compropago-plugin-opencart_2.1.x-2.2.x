...
class ControllerPaymentCppayment extends Controller {
  ...
  /**
   * Render main providers selection in checkout
   * 
  ...
  public function index() {
    ...
    $client = new Client(
      ...
      $this->config->get('cppayment_mode') == '1' ? true : false
    );
  ...
  public function confirm() {
    ...
    try {
      ...
      $json = [
        'status' => 'success',
        'redirect' => htmlspecialchars_decode($this->url->link('payment/cppayment/success', 'cp_id=' . $response->id , 'SSL'))
      ];
  ...
  public function webhook(){
    ...
    $publickey = $this->config->get('cppayment_public_key');
    $privatekey = $this->config->get('cppayment_private_key');
  ...
  private function addConfig(&$data, $client) {
    ...
    $providers = $client->api->listProviders((float) $order_info['total'], $order_info['currency_code']);
    $final = [];
    ...
    $data['show_logos'] = $this->config->get('cppayment_show_logos') == '1' ? true : false;
  }
}